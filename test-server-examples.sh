...
# 1. Health check
echo "1. Health check:"
curl -s http://localhost:5522/health | jq .
echo ""
...
curl -s http://localhost:5522/s5/fs/ | jq .
echo ""

# 7. List subdirectory
echo "7. Listing subdirectory (/data/):"
curl -s http://localhost:5522/s5/fs/data/ | jq .
echo ""

# 8. Delete a file
echo "8. Deleting a file:"
curl -X DELETE http://localhost:5522/s5/fs/test.txt -s | jq .
echo ""
